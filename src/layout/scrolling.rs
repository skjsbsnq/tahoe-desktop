...
niri_render_elements! {
    ScrollingSpaceRenderElement<R> => {
        ...
        MinimizeWindow = MinimizeWindowAnimationRenderElement,
...
impl<W: LayoutElement> ScrollingSpace<W> {
    ...
    fn apply_lease_event(&mut self, event: LeaseEvent<W::Id>) {
        match event {
            LeaseEvent::Suppress(id) => {
                if let Some(tile) = self.tiles_mut().find(|tile| tile.window().id() == &id) {
                    tile.apply_restore_visibility_lease();
                }
            }
            LeaseEvent::Reveal(id) => {
                if let Some(tile) = self.tiles_mut().find(|tile| tile.window().id() == &id) {
                    tile.release_restore_visibility_lease();
                }
            }
        }
    }
    ...
    pub fn has_visible_windows(&self) -> bool {
        self.columns.iter().any(Column::has_visible_windows)
    }
    ...
    pub fn active_window(&self) -> Option<&W> {
        ...
        if col.tiles[col.active_tile_idx].window().is_minimized() {
            return None;
        }
    ...
    pub fn remove_tile_by_idx(
        ...
        let id = self.columns[column_idx].tiles[tile_idx]
            .window()
            .id()
            .clone();
        if self
    ...
    pub fn activate_window(&mut self, window: &W::Id) -> bool {
        ...
        if let Some(tile_idx) = column.position(window) {
            column.tiles[tile_idx].window_mut().set_minimized(false);
        }
    ...
    pub fn set_minimized(
        ...
        window: &W::Id,
        minimized: bool,
        _animation_rect: Option<&MinimizeRect>,
    ) -> bool {
        ...
        let changed = self.columns[column_idx].tiles[tile_idx]
            .window_mut()
        ...
        if minimized {
            ...
            if let Some(event) = self.minimize_restore.reverse_to_minimize(
                ...
                self.apply_lease_event(event);
            } else if let Some(event) = self.minimize_restore.clear(window) {
                self.apply_lease_event(event);
            } else {
                self.columns[column_idx].tiles[tile_idx].release_restore_visibility_lease();
            }
            ...
            {
                self.interactive_resize = None;
                self.columns[column_idx].tiles[tile_idx]
                    .window_mut()
                    .cancel_interactive_resize();
            }

            if self.columns[column_idx].active_tile_idx == tile_idx {
                if let Some(idx) = self.columns[column_idx].first_visible_tile_idx() {
                    self.columns[column_idx].activate_idx(idx);
                }
            }

            if self.columns[self.active_column_idx].has_visible_windows() {
                return true;
            }

            if let Some(idx) = self.columns.iter().position(Column::has_visible_windows) {
    ...
    /// active for `window`; the controller's retarget is keyed by id. Mirrors
    /// the anchor consumption used at minimize/restore start.
    pub fn retarget_minimize_anchor(
        &mut self,
        window: &W::Id,
        rect: Option<super::coords::OutputLocalRect>,
    ) -> bool {
        self.minimize_restore.retarget(window, rect)
    }
    ...
    pub fn minimize_with_snapshot(
        ...
        if column.tiles[tile_idx].window().is_minimized() {
            return false;
        }

        if self.minimize_restore.has_restore(window) {
            return self.set_minimized(window, true, animation_rect);
        }
        ...
        let (snapshot, tile_view_pos) = {
            ...
            let snapshot = tile.render_animation_snapshot(
                renderer,
                xray,
    ...
    pub fn restore_with_snapshot(
        ...
        if self.minimize_restore.has_minimize(window) {
            return self.set_minimized(window, false, animation_rect);
        }

        if animation_rect.is_none() {
            return self.set_minimized(window, false, None);
        }

        if !column.tiles[tile_idx].window().is_minimized()
            || self.minimize_restore.has_restore(window)
        {
            return false;
        }

        if !self.set_minimized(window, false, animation_rect) {
            return false;
        }
            ...
            .for_each_overlay(|id, direction, animation| {
                lifecycle_overlays.push(LifecycleOverlayObservation {
                    kind: match direction {
                        LifecycleAnimDirection::Minimize => LifecycleOverlayKind::Minimize,
                        LifecycleAnimDirection::Restore => LifecycleOverlayKind::Restore,
                    },
        ...
        for (col, col_x) in self.columns_in_display_order() {
                ...
                && col.sizing_mode().is_normal()
            {
                ...
                if let Some(idx) = col.tab_indicator.hit(
                    ...
                    if col.tiles[idx].window().is_minimized() {
                        continue;
                    }
...
impl<W: LayoutElement> Column<W> {
    ...
    fn first_visible_tile_idx(&self) -> Option<usize> {
        self.tiles
            .iter()
            .position(|tile| !tile.window().is_minimized())
    }
