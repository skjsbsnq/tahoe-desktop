...
impl LayoutElement for TestWindow {
    ...
    fn set_minimized(&mut self, minimized: bool) -> bool {
        let changed = self.0.is_minimized.get() != minimized;
...
enum Op {
    ...
    RestoreWindow(#[proptest(strategy = "1..=5usize")] usize),
    SetFullscreenWindow {
        #[proptest(strategy = "1..=5usize")]
        window: usize,
        is_fullscreen: bool,
...
fn window_ipc_state(
    ...
    id: usize,
) -> (bool, Option<WorkspaceId>, WindowLayout) {
    let mut rv = None;
    layout.with_windows(|win, _output, ws_id, window_layout| {
        if *win.id() == id {
            rv = Some((win.is_minimized(), ws_id, window_layout));
        }
    });
    rv.unwrap()
}

fn assert_stable_window_layout(
    before: &(bool, Option<WorkspaceId>, WindowLayout),
    after: &(bool, Option<WorkspaceId>, WindowLayout),
) {
    assert_eq!(after.1, before.1);
    assert_eq!(
        after.2.pos_in_scrolling_layout,
        before.2.pos_in_scrolling_layout
    );
    assert_eq!(after.2.tile_size, before.2.tile_size);
    assert_eq!(after.2.window_size, before.2.window_size);
    assert_eq!(
        after.2.window_offset_in_tile,
        before.2.window_offset_in_tile
    );
}

#[test]
fn minimize_restore_tiling_window_keeps_ipc_layout() {
...
#[test]
fn minimize_restore_with_rect_keeps_ipc_layout() {
    let mut layout = check_ops([
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams::new(1),
...
fn minimize_restore_floating_window_keeps_position_and_size() {
    ...
    layout.move_floating_window(
        Some(&1),
        PositionChange::SetFixed(80.),
        PositionChange::SetFixed(90.),
        false,
    );
    layout.verify_invariants();

    let before = window_ipc_state(&layout, 1);
    assert_eq!(layout.focus().map(|win| *win.id()), Some(1));

    assert!(layout.minimize_window(&1));
    layout.verify_invariants();

    let minimized = window_ipc_state(&layout, 1);
    assert!(minimized.0);
    assert_eq!(&minimized.2, &before.2);
    assert_eq!(layout.focus().map(|win| *win.id()), None);

    assert!(layout.restore_window(&1));
    layout.verify_invariants();

    let restored = window_ipc_state(&layout, 1);
    assert!(!restored.0);
