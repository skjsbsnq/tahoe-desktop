...
impl<W: LayoutElement> FloatingSpace<W> {
    ...
    pub fn active_window_visual_rectangle(&self) -> Option<Rectangle<f64, Logical>> {
        ...
        let (tile, offset) = self
            .tiles_with_offsets()
            .find(|(tile, _)| tile.window().id() == active && !tile.window().is_minimized())?;
    ...
    pub fn activate_window(&mut self, id: &W::Id) -> bool {
        ...
        self.tiles[idx].window_mut().set_minimized(false);
    ...
    pub fn set_minimized(
        ...
        let changed = self.tiles[idx].window_mut().set_minimized(minimized);
        if !changed {
            return false;
        }
        ...
        if minimized {
            ...
            if Some(id) == self.active_window_id.as_ref() {
    ...
    pub fn minimize_with_snapshot(
        ...
        if !self.set_minimized(id, true, animation_rect) {
            return false;
        }
    ...
    pub fn restore_with_snapshot(
        ...
        if !self.set_minimized(id, false, animation_rect) {
            return false;
        }
    ...
    pub fn refresh(&mut self, is_active: bool, is_focused: bool) {
        ...
        for tile in &mut self.tiles {
            ...
            let mut is_active =
                !win.is_minimized() && is_active && Some(win.id()) == active.as_ref();
    ...
    pub fn verify_invariants(&self) {
        ...
        if let Some(id) = &self.active_window_id {
            ...
            assert!(
                ...
                "active window must not be minimized"
            );
        } else {
            assert!(
                self.tiles.is_empty() || !self.has_visible_windows(),
                "only an empty or fully minimized floating space may have no active window"
            );
        }
