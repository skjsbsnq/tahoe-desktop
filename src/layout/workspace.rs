    ...
    pub fn verify_invariants(&self, move_win_id: Option<&W::Id>) {
        ...
        if !self.floating.has_visible_windows() {
            assert!(
                ...
                "when floating has no visible windows it must not be active or raised"
            );
        } else if self.scrolling.is_empty() {
            assert!(
                self.floating_is_active.get(),
                "when scrolling is empty but floating has visible windows, floating should be active"
            );
        } else if !self.scrolling.has_visible_windows() {
            assert!(
                self.floating_is_active.get(),
                "when scrolling has no visible windows but floating does, floating should be active"
            );
        }
