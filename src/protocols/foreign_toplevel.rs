...
pub fn refresh(state: &mut State) {
    ...
    if let Some((identifier, window, is_minimized, output)) = focused {
...
mod tests {
    ...
    #[test]
    fn to_state_vec_includes_minimized_state() {
        let states = ToplevelStateSet::default();
        let rv = to_state_vec(&states, false, true);
    ...
    #[test]
    fn to_state_vec_can_report_minimized_and_activated() {
        let states = ToplevelStateSet::default();
        let rv = to_state_vec(&states, true, true);
