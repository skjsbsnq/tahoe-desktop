vec4 genie_color(vec2 output_pos) {
    ...
    float top_progress = pow(morph, 1.35);
    float bottom_progress = 1.0 - pow(1.0 - morph, 1.9);

    float window_top = window_pos.y;
    float window_bottom = window_pos.y + window_size.y;
    float target_top = target_pos.y;
    float target_bottom = target_pos.y + target_size.y;
    ...
    float waist = 1.0 - sin(v * 3.14159265) * morph * 0.08;
    half_width = max(half_width * waist, 0.5);
    ...
    float end_fade = 1.0 - smoothstep(0.85, 1.0, morph);
